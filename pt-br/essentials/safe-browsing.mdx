...
Quando você vê o ícone de cadeado (🔒) e `https://` na barra de endereço, significa que a conexão está criptografada usando **TLS (Transport Layer Security — Segurança da Camada de Transporte)**.
...
3. **Autenticação (Authentication)**: A identidade do site é verificada por uma Autoridade Certificadora (CA — Certificate Authority).
...
- **Provedores DNS recomendados**: Cloudflare (`1.1.1.1`), Google Public DNS (`8.8.8.8`), Quad9 (`9.9.9.9` — também bloqueia domínios maliciosos conhecidos).
